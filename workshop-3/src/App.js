...
const CommentOverlay = () => {
  ...
  const { comments, commentInput } = useSelector((s) => (
    {
      comments: s.comments,
      commentInput: s.commentInput
    }
  ))
...
const Marker = ({ marker }) => {
  ...
  const { markerScale, textScale } = useSelector((s) => ({
    markerScale: s.mapConfig.markerScale,
    textScale: s.mapConfig.markerTextScale
  }), shallowEqual);
  useEffect(() => {
...
const App = () => {
  ...
  const { markers,  markerScale, textScale } = useSelector((s) => ({
    markers: s.markers,
    markerScale: s.mapConfig.markerScale,
    textScale: s.mapConfig.markerTextScale
  }), shallowEqual);
  ...
  const markerList = markers.map((marker) => <Marker marker={marker} />)
